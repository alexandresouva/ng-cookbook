...
6. **Component & Integration Testing**: Use the unified `TestHelper` class from `@testing/test-helper/test-helper` to query, trigger, and dispatch actions in component tests. Always target `data-testid` attributes in HTML templates for element selection.
