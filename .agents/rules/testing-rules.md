...
9. **End-to-End (E2E) Testing Rules (Playwright)**:
   - **Page Object Model (POM)**: Represent page views and complex widgets using classes in `e2e/page-objects/`. Define locators inside the constructor and expose visual interactions as async methods.
   - **Playwright Fixtures**: Never instantiate page objects using `new PageObject(page)` inside spec files. Declare page objects as custom fixtures in `e2e/fixtures/e2e-fixtures.ts` and inject them directly into test parameters.
   - **Hybrid Locator Strategy**: Prioritize user-visible semantic locators (e.g. `getByRole()`, `getByLabel()`) to verify implicit accessibility (a11y). Fallback to test IDs (`getByTestId()`) for list records, icons, layout structures, or volatile text properties to ensure test stability.
   - **Route Mocking**: Use Playwright's native `page.route()` to intercept APIs during E2E mocked integration runs. Populate mocked routes using the shared factories under `@testing/factories/` to avoid data duplication.
